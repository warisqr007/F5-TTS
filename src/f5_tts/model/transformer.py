...
from .compile import no_compile
...
class StreamingMultiheadAttention(StreamingModule[_MHAState]):
    ...
    def __init__(
        ...
        right_context: tp.Optional[int] = None,
        rope: tp.Optional[RotaryEmbedding] = None,
        pe_attn_head: tp.Optional[int] = None,
        ...
        in_proj = nn.Linear(input_dim, mult * out_dim, bias=False, **factory_kwargs)
        # We try to follow the default PyTorch MHA convention, to easily compare results.
        self.in_proj_weight = in_proj.weight
        self.in_proj_bias = in_proj.bias
        self.out_proj = nn.Linear(
            embed_dim, mult * input_dim, bias=False, **factory_kwargs
        )
    ...
    def forward(self, query: torch.Tensor, key: torch.Tensor, value: torch.Tensor, tr: int = 0, mask = None):
        state = self._streaming_state
        T = query.shape[1]
        Tq = T - tr
        ...
        # cache past + CURRENT
        k_cache, v_cache, pos_k_cache = self._complete_kv(k[:, :, :Tq, :], v[:, :, :Tq, :])
        
        # append FUTURE (not cached)
        if tr > 0:
            k_all = torch.cat([k_cache, k[:, :, Tq:, :]], dim=2)
            v_all = torch.cat([v_cache, v[:, :, Tq:, :]], dim=2)
            # print("offset", offset)
            pos_k_fut = offset + torch.arange(Tq, Tq + tr, device=offset.device, dtype=torch.long)
            # print("pos_k_fut", pos_k_fut)
            pos_k = torch.cat([pos_k_cache, pos_k_fut], dim=0)
        else:
            k_all, v_all, pos_k = k_cache, v_cache, pos_k_cache
        ...
        if self.causal:
            ...
            # NEW: allow up to self.right_context into the future
            R = getattr(self, "right_context", 0)
            attn_bias = attn_bias & (delta >= -R)
        else:
            attn_bias = None
        # x = F.scaled_dot_product_attention(q, k, v, attn_bias, dropout_p=0.0)
        x = F.scaled_dot_product_attention(q, k_all, v_all, attn_bias, dropout_p=0.0)
...
class StreamingTransformerLayer(StreamingModule[_LayerState]):
    ...
    def _sa_block(self, x: torch.Tensor, tr: int = 0) -> torch.Tensor:
        if self.skip_self_attn:
            return x
        x_orig = x
        x = self.norm1(x)
        update = self.self_attn(x, x, x, tr=tr)
        return x_orig + self.layer_scale_1(update)

    def forward(self, x: torch.Tensor, tr: int = 0):
        with ExitStack() as stack:
            if x.device.type != 'cuda':
                stack.enter_context(no_compile())
            x = self._sa_block(x, tr=tr)
            x = self._ff_block(x)
            state = self._streaming_state
            if state:
...
class StreamingTransformer(StreamingModule[_TransformerState]):
    ...
    def forward(self, x: torch.Tensor, tr: int = 0, *args, **kwargs):
        B, T, C = x.shape
...
class ProjectedTransformer(StreamingContainer):
    ...
    def forward(self, x, tr=0, *args, **kwargs):
        if self.conv_layout:
            x = x.transpose(1, 2)
        if self.input_proj is not None:
            x = self.input_proj(x)
        z = self.transformer(x, tr=tr, *args, **kwargs)
        ys = []
