...
from f5_tts.model.rope import RotaryEmbedding
from f5_tts.model.streaming import StreamingContainer

from f5_tts.model.streaming_modules import (
    AdaLayerNorm_Final,
    ConvNeXtV2Block,
...
class TextEmbedding(StreamingContainer):
    def __init__(
...
class DiT(StreamingContainer):
    def __init__(
        ...
        *,
        ...
        self.rotary_embed = RotaryEmbedding(max_period=max_period)
    ...
    def forward(
        ...
        for block in self.transformer_blocks:
            ...
            #     x = torch.utils.checkpoint.checkpoint(self.ckpt_wrapper(block), x, t, mask, self.rotary_embed, use_reentrant=False)
            # else:
            #     x = block(x, t, mask=mask, rope=self.rotary_embed)
            if self.checkpoint_activations:
                # https://pytorch.org/docs/stable/checkpoint.html#torch.utils.checkpoint.checkpoint
                x = torch.utils.checkpoint.checkpoint(self.ckpt_wrapper(block), x, t, mask, use_reentrant=False)
            else:
...
if __name__ == "__main__":
    ...
    text = torch.randint(0, 256, (b, nt))
    time = torch.randint(0, 1000, (b,)).float()
    mask = torch.ones(b, n).bool()

    out = model(x, cond, text, time, mask)
    print(out.shape)
